...
describe("Milestone 3 facade", function()
  ...
  it("deep-merges maps, replaces lists, does not alias, and freezes config", function()
    ...
    assert.are.equal(1920, second.config.exactPresets[2].width)
  ...
  it("retains an active Composition live timer across a stop fault and releases it on retry", function()
    local instance = Anodyne.new({ hs = driver.hs }):start()
    driver:triggerHotkey({ "ctrl", "alt", "cmd" }, "c")
    local retained = instance.compositionLiveTimer
    assert.is_true(retained._state.active)
    driver:setPersistentLifecycleFault("timer.stop")
    local _, errors = instance:stop()
    assert.is_table(errors)
    assert.matches("compositionLiveTimer.stop", table.concat(errors, "\n"))
    assert.are.equal(retained, instance.compositionLiveTimer)
    driver:clearLifecycleFaults()
    assert.is_nil(select(2, instance:stop()))
    assert.is_nil(instance.compositionLiveTimer)
    assertNoResources(driver)
  end)
